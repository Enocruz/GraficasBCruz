...
Mesh mesh;
Dephtbuffer newBuffer;
ShaderProgram shProgram, shProgramPuerco, shNuevo;
Transform geometria, geometria2;
Camera _camera, camara2;
...
void Initialize() {
	...
	newBuffer.Create(2048);
	camara2.SetPosition(0.0f, 1.0f, 10.0f);
	camara2.SetOrthographic(100.0f, 1.0f);
	shNuevo.CreateProgram();
	shNuevo.SetAttribute(0, "VertexPosition");
	shNuevo.AttachShader("Depth.frag", GL_FRAGMENT_SHADER);
	shNuevo.AttachShader("Depth.vert", GL_VERTEX_SHADER);
	shNuevo.LinkProgram();
	...
	shProgram.SetUniformi("ShadowMap", 0);
...
void GameLoop() {
	...
	shNuevo.Activate();
	shNuevo.SetUniformMatrix("mvpMatrix", _camera.GetViewProjection()*geometria.GetModelMatrix());
	mesh.Draw(GL_TRIANGLES);
	shNuevo.SetUniformMatrix("mvpMatrix", _camera.GetViewProjection()*geometria2.GetModelMatrix());
	mesh.Draw(GL_TRIANGLES);
	shNuevo.Deactivate();
	...
	shProgram.SetUniformf("CameraPosition", _camera.GetPosition().x, _camera.GetPosition().y, _camera.GetPosition().z);
	//shProgram.SetUniformMatrix("LightVPMatrix", camara2.GetViewProjection());
	glActiveTexture(GL_TEXTURE0);
	myTexture2.Bind();
	mesh.Draw(GL_TRIANGLES);
	glActiveTexture(GL_TEXTURE0);
