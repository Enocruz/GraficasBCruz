...
{
	// Realizar la correccin de perspectiva. Coordenadas en rango [-1, 1] -> Normalized Device Space
	...
	// La profundidad del pixel desde la actual cmara (ya transformado) est en projCoords.z
...
void main()
{
	...
	FragColor = mix(texture2D(DiffuseTexture, IterpolatedTexCoord), texture2D(DiffuseTexture2, IterpolatedTexCoord),0.3f) * vec4(phong,1.0f);
}